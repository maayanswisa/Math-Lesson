...
path = Path(__file__).resolve().parents[1] / "src/components/quiz/SciCalculator.jsx"

# Pure ASCII source for the .jsx file (Unicode only via JS \\u escapes)
src = r"""import { useState } from 'react';
...
assert all(b < 128 or True for b in data)  # utf8 may have multi-byte from... none, all ascii
assert max(data) < 128, f"non-ascii at {data.index(bytes([max(data)]))}"
text = path.read_text(encoding="utf-8")
assert "\\u221A" in text
assert "setOpen" not in text
print("OK ascii-only jsx", len(text))
