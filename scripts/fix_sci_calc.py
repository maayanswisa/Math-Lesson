...
# Entire file as ASCII Python string; JS gets real Unicode via chr / escapes
...
/** Scientific calculator — always open, organized keypad. */
  ...
  return (
    <div className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-black/10" dir="ltr">
      <div className="mb-3 flex items-center justify-between gap-2" dir="rtl">
        <span className="text-sm font-semibold text-[var(--color-ink)]">TITLE_HE</span>
      ...
      <p className="mb-1.5 text-left text-[10px] font-medium uppercase tracking-wider text-[var(--color-slate)]">
        SCI_LABEL
      </p>
...
text = content.replace("TITLE_HE", title).replace("SCI_LABEL", sci).replace("KEY_LABEL", key).replace("BKSP", bksp)
# Decode JS \uXXXX for SQRT etc - in raw string they are literal backslash-u which IS correct for JS
out = ROOT / "src/components/quiz/SciCalculator.jsx"
out.write_text(text, encoding="utf-8")
# verify utf8
data = out.read_text(encoding="utf-8")
assert title in data
assert "\\u221A" in data  # JS escape in source
assert "setOpen" not in data
print("ok", len(data))
